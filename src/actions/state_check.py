...
def get_state_params(ctx, params):
  if isinstance(params, str):
    ...
    state_path = path.join(ctx.sub_dir, ctx.vars["BASE_DIR"], DEFAULT_STATE_FILENAME)
  else:
    ...
    if path_ is None:
      ...
      if re.fullmatch(".+/$", path_) is not None:
        state_path = path.join(ctx.sub_dir, path_, DEFAULT_STATE_FILENAME)
      else:
        state_path = path.join(ctx.sub_dir, path_)
